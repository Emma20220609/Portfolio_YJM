...
<tr>
	<td colspan="4" align="center" style="height:50px;">
		<a href="../board/qna.jsp?kind=<%= kind %>&no=<%= no %>&page=<%= cpage %>&search_key=<%= search_key %>">글목록</a>
		<%
			if(login_id != null)
			{
				if(login_autho.equals("A") || login_no.equals(user_no))
				{
					%>
						&nbsp;|&nbsp;
						<a href="../modify/qnamodify.jsp?no=<%= no %>&kind=<%= kind %>&page=<%= cpage %>&search_key=<%= search_key %>">글수정</a>
						&nbsp;|&nbsp;
						<a href="../delete/qnadelete.jsp?no=<%= no %>&kind=<%= kind %>&page=<%= cpage %>&search_key=<%= search_key %>">글삭제</a>
					<%
				}
			}
		%>
