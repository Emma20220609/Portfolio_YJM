...
<script>
	...
	function DoSubmit()
	{
         ...
         if( !idReg.test( $("input[name=userid]").val() ) ) 
         {
             ...
             alert("아이디는 6~20자 영문자 또는 숫자이어야 합니다.");
             return;
         }
	...
	function onlyAlphabet() 
	{
        ...
        if ($("#userid").val().indexOf(" ") >= 0) 
        {
            ...
            //document.joinFrm.userid.select();
            $("#userid").val().focus();
            $("#userid").val().select();
            return false;
        }
	...
	function onlyNum()
	{
		for (var i = 0; i < document.joinFrm.phone.value.length; i++) {
          ch = document.joinFrm.phone.value.charAt(i)
          if (!(ch >= '0' && ch <= '9')) {
              alert("연락처는 숫자만 입력 가능합니다.");
              document.getElementById("mobile2").value = "";
              //document.joinFrm.mobile2.focus();
              //document.joinFrm.mobile2.select();
            $("#mobile2").val().focus();
            $("#mobile2").val().select();
              return false;
          }
	...
	<div id="container">
		...
		<div class="sv_wrap K01">
			<p class="sv_tit"><img src="../image/tit_K07.png" alt="MENU"></p>
			<p class="page_path"><a href="index.jsp">HOME</a><span class="arr"></span>회원가입</p>
		</div>
		<!-- Content-->
		<div class="content">
			...
			<div class="outer tint">
				...
				<!--회원가입 -->
					<div id="contents">
						<form id="joinForm" name="joinForm" action="joinok.jsp" method="post">
							<div class="xans-element- xans-member xans-member-login">
								...
								<table border="0">
									<tbody>
										<tr>
											<th scope="row">아이디 </th>
						                	<td><input must="아이디를 입력하세요" id="userid" name="userid" class="inputTypeText" type="text" onkeyup="onlyAlphabet()"> <span id="idMsg"></span> (영문소문자/숫자, 4~16자)</td>
						            	</tr>
										<tr>
											<th scope="row">비밀번호</th>
						                	<td><input must="비밀번호를 입력하세요" id="userpw" name="userpw" type="password" > (영문 대소문자/숫자/특수문자 중 2가지 이상 조합, 6자~20자)</td>
						            	</tr>
										<tr>
											<th scope="row">비밀번호 확인 </th>
						                	<td><input must="비밀번호를 입력하세요" id="pwcheck" name="pwcheck"  type="password"> <span id="pwConfirmMsg"></span> </td>
						            	</tr>
										<tr>
										<th scope="row" id="username" name="username">이름 </th>
							                <td>
							                    <span id="nameContents"><input must="이름을 입력하세요"  type="text" name="name" id="name" maxlength="20"></span>
										...
										<tr class="">
							                ...
							                <td>
												...
												</select>-<input must="전화번호를 입력하세요" id="mobile2" name="mobile2" maxlength="4" type="text" onkeyup="onlyNum()">
														-<input must="전화번호를 입력하세요" id="mobile3" name="mobile3" maxlength="4" type="text" onkeyup="onlyNum()">
											</td>
							            </tr>
										<tr>
											<th scope="row">이메일 </th>
							                <td>
							                	<input must="이메일을 입력하세요" id="email" name="email"  type="text"> <span id="emailMsg"></span>
											</td>
							            </tr>
									</tbody>
								</table>
							...
							<h3>이용약관 동의</h3>
								<div class="agreeArea">
									<div class="agree">
										<div class="fr-view fr-view-mall-agreement"><p>제1조(목적)<br>이 약관은 OO 회사(전자상거래 사업자)가 운영하는 OO 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.
																									<br>※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」</p>
																									<p>제2조(정의)<br>① “몰”이란 OO 회사가 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
																									...
																									<br>&nbsp; 3. 기타 “몰”이 정하는 업무<br>② “몰”은 재화 또는 용역의 품절 또는 기술적 사양의 변경 등의 경우에는 장차 체결되는 계약에 의해 제공할 재화 또는 용역의 내용을 변경할 수 있습니다. 이 경우에는 변경된 재화 또는 용역의 내용 및 제공일자를 명시하여 현재의 재화 또는 용역의 내용을 게시한 곳에 즉시 공지합니다.
																									<br>③ “몰”이 제공하기로 이용자와 계약을 체결한 서비스의 내용을 재화등의 품절 또는 기술적 사양의 변경 등의 사유로 변경할 경우에는 그 사유를 이용자에게 통지 가능한 주소로 즉시 통지합니다.
																									<br>④ 전항의 경우 “몰”은 이로 인하여 이용자가 입은 손해를 배상합니다. 다만, “몰”이 고의 또는 과실이 없음을 입증하는 경우에는 그러하지 아니합니다.</p><p>제5조(서비스의 중단) 
																									...
																									<br>③ 사업종목의 전환, 사업의 포기, 업체 간의 통합 등의 이유로 서비스를 제공할 수 없게 되는 경우에는 “몰”은 제8조에 정한 방법으로 이용자에게 통지하고 당초 “몰”에서 제시한 조건에 따라 소비자에게 보상합니다. 다만, “몰”이 보상기준 등을 고지하지 아니한 경우에는 이용자들의 마일리지 또는 적립금 등을 “몰”에서 통용되는 통화가치에 상응하는 현물 또는 현금으로 이용자에게 지급합니다.</p><p>제6조(회원가입) 
																									...
																									<br>④ 회원은 회원가입 시 등록한 사항에 변경이 있는 경우, 상당한 기간 이내에 “몰”에 대하여 회원정보 수정 등의 방법으로 그 변경사항을 알려야 합니다.</p><p>제7조(회원 탈퇴 및 자격 상실 등) 
								...
								<div class="agreeArea">
								        ...
								        <p class="check"><span>개인정보 수집 및 이용에 동의하십니까?</span> <input must="개인정보 수집 및 이용 방침에 동의하세요" id="agree" name="agree" class="ec-base-chk" value="1" type="checkbox"><label for="agree_privacy_check0">동의함</label></p>
								</div>
			
			
								<div class="btnCommon type2">
									<a href="javascript:DoSubmit();" class="btnBlack">Join</a>
									<a href="#none" class="btnGray" onclick="history.go(-1); return false;">Cancel</a>
			   					</div>
								<!-- 회원 가입 정보 확인 레이어 -->
							</div>
						</form>
